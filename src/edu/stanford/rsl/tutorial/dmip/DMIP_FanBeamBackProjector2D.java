...
public class DMIP_FanBeamBackProjector2D {
	...
	public void setSinogramParams(Grid2D sino, double focalLength, double maxRot)
	{
		...
		double halfFanAngle = Math.atan((detectorLength/2)/focalLength);//TODO
	...
	public Grid2D applyCosineWeights(Grid2D sino)
	{
		...
		for(int i=0; i<detectorPixels; ++i){
			...
			float value = (float)(focalLength / (Math.sqrt((Math.pow(focalLength,2)+ Math.pow(s, 2)))));
			cosineKernel.setAtIndex(i, value);//TODO
		}
		...
		for (int t = 0; t < detectorPixels; ++t) {
			...
			for (int b = 0; b < numProjs; ++b) {
				...
				if (beta <= 2 * (delta - gamma)) {
					float val = (float)(Math.pow(Math.sin((Math.PI*beta)/(4.0*(delta-gamma))), 2)); //TODO
					if (Double.isNaN(val)){
						continue;
					}
					...
					float val = (float)(Math.pow(Math.sin((Math.PI*(Math.PI+(2*delta)-beta))/(4*(delta+gamma))), 2)) ;//TODO
					if (Double.isNaN(val)){
						continue;
					}
					parker.setAtIndex(t, b , val);
	...
	public static void main(String arg[])
	{
		...
		String filename = "/proj/i5fpctr/FranziFAU/CONRAD/src/edu/stanford/rsl/tutorial/dmip/Sinogram0.tif";
